...
const Rating = ({ priority }: RatingProps) => {
  return (
    <div className="flex flex-col items-center justify-center gap-3 md:flex-row md:items-start">
      {/* AVATARS */}
      <div className={`-space-x-5 avatar-group justy-start max-sm:hidden`}>
        {avatars.map((image, i) => (
          <div className="w-12 h-12 avatar max-sm:w-10 max-sm:h-10" key={i}>
            <Image
              src={image.src}
              alt={image.alt}
              priority={priority}
      ...
      <div className="flex flex-col items-center justify-center gap-1 md:items-start">
        ...
        <div className="text-base text-base-content/80 max-sm:text-sm">
          <span className="font-semibold text-base-content">542</span> creators
          likes it!
        </div>
      </div>
    </div>
